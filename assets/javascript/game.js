...
if(playerHP >= 0)
{
    ...
    $(document).one("click", ".enemy", function(){
        ...
        if(enemyChosen == true)
        {
            ...
            var returnEnmSrc = enemies.filter(obj => obj.name == chosenEnemy ).map(obj => obj.src); // call values from array
            var returnEnmHP = enemies.filter(obj => obj.name == chosenEnemy ).map(obj => obj.hp); // call values from array
